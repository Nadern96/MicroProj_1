...
{
	//RW = 0 to write
	DIO_vid_WritePinVal(LCD_Control,LCD_RW,LOW);

	//RS = 0 for command
	DIO_vid_WritePinVal(LCD_Control,LCD_RS,LOW);
	...
	DIO_vid_WritePinVal(LCD_Control,LCD_E,HIGH);

	//leave E= 1 (opened) for 2milliseconds until data is displayed
	delay_ms(2);

	DIO_vid_WritePinVal(LCD_Control,LCD_E,LOW);
}
...
void LCD_vidWriteData (u8 data)
{
	...
	//RS = 0 for command
	DIO_vid_WritePinVal(LCD_Control,LCD_RS,HIGH);
